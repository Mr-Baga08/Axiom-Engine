...
def eval_llm(prompt: str, temperature: float = 0.0) -> str:
    """
    Relevance evaluator using Gemini Flash.
    Gemini is used here (not Claude) because eval calls happen per-chunk
...
@observe(name="crag_retrieve", tags=["rag", "retrieval"], capture_output=False)
def crag_retrieve(
    question: str,
    ...
    if pass1_chunks:
        ...
        _maybe_augment_graphrag(result, question)
        return result
...
def _maybe_augment_graphrag(result: CRAGResult, question: str) -> None:
    """Append a GraphRAG community-summary chunk when CRAG yields fewer than 2 chunks."""
    if len(result.chunks) >= 2:
        return
    if os.getenv("USE_GRAPHRAG", "false").lower() != "true":
        return
    try:
        from .graphrag_retriever import graphrag_query
        text = graphrag_query(question)
        result.chunks.append(ScoredChunk(
            text=text,
            metadata={"source": "graphrag", "page": 0},
            relevance=0.75,
            tamper_ok=True,
        ))
        result.sources.append({"source": "graphrag", "page": 0, "relevance": 0.75})
        logger.info("GraphRAG fallback appended for low-chunk result")
    except Exception as exc:
        logger.warning("GraphRAG fallback failed: %s", exc)
